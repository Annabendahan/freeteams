...
addsCourse = () => {
    let token = "Bearer " + localStorage.getItem("jwt")
        ...
        axios.post('/api/courses', {course: course},  { 'content-type': 'application/x-www-form-urlencoded', headers: {'Authorization': token }})
            .then(response => {
                console.log(response);
                 const courses = update(this.state.courses, {
      $splice: [[0, 0, response.data]]
...
displayDoneHandler = () =>{
  ...
  render() {
    ...
    <button className="newCourseButton" onClick={(e) => this.addNewCourse(e)} >
